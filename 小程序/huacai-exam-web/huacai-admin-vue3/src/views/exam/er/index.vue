    ...
    <div class="app-container">
        ...
        <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
            ...
            <el-form-item label="是否正确" prop="isCorrect">
                <el-select style="width: 200px" v-model="queryParams.isCorrect" placeholder="请选择是否正确" clearable>
                    <el-option
                            v-for="dict in is_correct"
                            :key="dict.value"
                            :label="dict.label"
                            :value="dict.value"
                    />
        ...
        <el-table @row-click="clickRow" ref="table" highlight-current-row
                  border v-loading="loading" :data="erList" @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="55" align="center"/>
            <el-table-column label="序号" align="center" type="index" :index="indexMethod"/>
            <el-table-column label="考试名称" align="center" prop="examTitle"/>
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template #default="scope">
                    <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                               v-hasPermi="['exam:er:edit']">修改
                    </el-button>
                    <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                               v-hasPermi="['exam:er:remove']">删除
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <vxe-modal :title="title" v-model="open" width="500px" show-maximize showFooter resize>
            <el-form ref="erRef" :model="form" :rules="rules" label-width="80px">
                ...
                <el-form-item label="是否正确" prop="isCorrect">
                    <el-select v-model="form.isCorrect" placeholder="请选择是否正确">
                        <el-option
                                v-for="dict in is_correct"
                                :key="dict.value"
                                :label="dict.label"
...
<script setup name="Er">
import {listEr, getEr, delEr, addEr, updateEr} from "@/api/exam/er"
...
const data = reactive({
    ...
    rules: {
        ...
        isCorrect: [
            {required: true, message: "是否正确不能为空", trigger: "change"}
        ],
        userId: [
...
const getList = () => {
    loading.value = true
    listEr(queryParams.value).then(response => {
        erList.value = response.rows
...
const handleUpdate = (row) => {
    reset()
    const _erId = row.erId || ids.value
    getEr(_erId).then(response => {
...
/** 提交按钮 */
const submitForm = () => {
    proxy.$refs["erRef"].validate(valid => {
        if (valid) {
            if (form.value.erId != null) {
                updateEr(form.value).then(response => {
                    proxy.$modal.msgSuccess("修改成功")
                    open.value = false
                    getList()
                })
            } else {
                addEr(form.value).then(response => {
...
const handleDelete = (row) => {
    const _erIds = row.erId || ids.value
...
/** 导出按钮操作 */
const handleExport = () => {
    proxy.download('exam/er/export', {
        ...queryParams.value
    }, `er_${new Date().getTime()}.xlsx`)
}
