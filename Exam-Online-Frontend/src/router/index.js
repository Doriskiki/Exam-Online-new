...
const routes = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        path: "/faceRegister",
        component: () => import("../components/FaceRegister"),
      },
      // 人脸管理(all)
      {
        path: "/faceManage",
        component: () => import("../components/FaceManage"),
      },
      // 人脸测试(开发用)
      {
        path: "/faceTest",
        component: () => import("../components/FaceTest"),
      },
    ],
  },
  //考试界面(管理员和学生)
...
router.beforeEach((to, from, next) => {
  ...
  // 定义不需要验证的角色路由检查函数
  const checkAuth = (successValidation) => {
    // 如果离线，直接放行 (Offline-First)
    if (!navigator.onLine) {
      return next();
    }
    axios
      ...
      .then((resp) => {
        if (resp.data.code === 200 && successValidation(resp.data.data)) {
          next();
        } else {
          next("/index");
        }
  ...
  if (
    ...
    return checkAuth((data) => data.roleId === "3");
  }
  ...
  if (
    ...
    return checkAuth((data) => data.roleId === "3" || data.roleId === "2");
  }
  ...
  if (
    ...
    return checkAuth((data) => data.roleId !== "2");
  }
