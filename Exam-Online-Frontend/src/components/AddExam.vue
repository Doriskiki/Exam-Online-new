  ...
  <el-container>
    ...
    >
      <el-row>
        ...
        ></el-input>
        <el-button type="primary" @click="addQuToFree" style="float: right"
          >确认{{ selectedTable.length }}项</el-button
        >
      </el-row>
...
export default {
  ...
  methods: {
    ...
    getBankInfo() {
      this.$http
        .get(this.API.getBankHaveQuestionSumByType, {
          params: {
            pageNo: 1,
            pageSize: 9999
          }
    ...
    delQuestion(questionId) {
      this.addExamQuestion2.forEach((item, index) => {
        if (item.questionId === questionId)
          this.addExamQuestion2.splice(index, 1);
      });
    },
    //题目类型变化
    typeChange(val) {
    ...
    addExam() {
      this.$refs["examInfoForm"].validate(valid => {
        if (
          valid &&
          (this.addExamQuestion.length !== 0 ||
            this.addExamQuestion2.length !== 0)
        ) {
          //构造数据对象(考试信息)
          ...
          if (this.examAuthority === 2) {
            //考试密码
            if (this.examPassword === "") {
              // 当前用户选择了需要密码权限,但是密码为空
          ...
          if (
            this.makeModel === 1 &&
            !this.addExamQuestion.some(item => item.bankId === "")
          ) {
