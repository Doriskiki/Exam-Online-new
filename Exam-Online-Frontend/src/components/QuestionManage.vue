  ...
  <el-container>
    ...
    <el-main>
      ...
      >
        ...
        <el-table-column align="center" label="难度">
          <template slot-scope="scope">
            <el-tag effect="dark" type="success" v-if="scope.row.level === 1"
              >简单</el-tag
            >
            <el-tag
              effect="dark"
        ...
        <el-table-column align="center" prop="quBankName" label="所属题库">
        </el-table-column>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button round type="primary" @click="updateQuestion"
          >确 定</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      this.addQuForm.images.map((item, index) => {
        //移除图片在表单中的数据
    ...
    handleUpdateRemove(file, fileList) {
      this.updateQuForm.images.map((item, index) => {
        //移除图片在表单中的数据
    ...
    handleAnswerRemove(file) {
      this.addQuForm.answer.images.map((item, index) => {
        //移除图片在表单中的数据
        if (item === file.response.data) this.addQuForm.images.splice(index, 1);
      });
    },
    ...
    handleUpdateAnswerRemove(file) {
      this.updateQuForm.answer.images.map((item, index) => {
        //移除图片在表单中的数据
        if (item === file.response.data)
          this.updateQuForm.images.splice(index, 1);
      });
    ...
    checkAnswer(checked, id) {
      if (checked) {
        if (
          this.addQuForm.questionType === 1 ||
          this.addQuForm.questionType === 3
        ) {
          //单选或者判断
          //当前已经有一个正确的选择了
    ...
    checkUpdateAnswer(checked, id) {
      if (checked) {
        if (
          this.updateQuForm.questionType === 1 ||
          this.updateQuForm.questionType === 3
        ) {
          //单选或者判断
          //当前已经有一个正确的选择了
    ...
    addQuestion() {
      this.$refs["addQuForm"].validate(valid => {
        if (
          ...
          return false;
        } else if (valid && this.addQuForm.questionType === 4) {
          //简答题 无标准答案直接发请求
          //当是简答题的时候需要清除answer
    ...
    updateQuestion() {
      this.$refs["updateQuForm"].validate(valid => {
        if (
          ...
          this.$message.error("必须有一个答案");
          return false;
        } else if (valid && this.updateQuForm.questionType === 4) {
          //简答题 无标准答案直接发请求
          //当是简答题的时候需要清除answer
          this.addQuForm.answer = [];
          this.$http
