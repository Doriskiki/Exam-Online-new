          ...
          <el-col :span="16">
            <el-card style="min-height: 500px">
              ...
              <div>
                ...
                <span v-if="questionInfo[curIndex].questionType === 1"
                  >【单选题】</span
                >
                <span v-else-if="questionInfo[curIndex].questionType === 2"
                  >【多选题】</span
                >
                <span v-else-if="questionInfo[curIndex].questionType === 3"
                  >【判断题】</span
                >
              ...
              <div
                v-if="
                  questionInfo[curIndex].images &&
                  questionInfo[curIndex].images.length > 0
                "
                style="margin-top: 15px"
              >
                <div
                  v-for="(url, imgIndex) in questionInfo[curIndex].images"
                  :key="imgIndex"
                  style="
              ...
              <div
                ...
                v-show="
                  questionInfo[curIndex].questionType === 1 ||
                  questionInfo[curIndex].questionType === 3
                "
              >
              ...
              >
                <el-input
                  ...
                  @input="onTextAnswerChange(curIndex, $event)"
                >
                </el-input>
              </div>
            ...
            <el-card>
              ...
              <div style="margin-top: 25px">
                ...
                <el-button
                  ...
                  :class="
                    questionInfo[item - 1].questionType === 1 &&
              ...
              <div style="margin-top: 25px">
                ...
                <el-button
                  ...
                  :class="
                    questionInfo[item - 1].questionType === 2 &&
                    userAnswer[item - 1] !== undefined
                      ? 'done'
                      : userAnswer[item - 1] === undefined
                      ? curIndex === item - 1
              ...
              <div style="margin-top: 25px">
                ...
                <el-button
                  ...
                  :class="
                    questionInfo[item - 1].questionType === 3 &&
                    userAnswer[item - 1] !== undefined
                      ? 'done'
                      : userAnswer[item - 1] === undefined
                      ? curIndex === item - 1
              ...
              <div style="margin-top: 25px">
                ...
                <el-button
                  ...
                  :class="
                    questionInfo[item - 1].questionType === 4 &&
                    userAnswer[item - 1] !== undefined
                      ? 'done'
                      : userAnswer[item - 1] === undefined
                      ? curIndex === item - 1
...
export default {
  ...
  methods: {
    ...
    getExamInfo() {
      this.$http
        .get(this.API.getExamInfoById, { params: this.$route.params })
        .then((resp) => {
          if (resp.data.code === 200) {
            this.examInfo = resp.data.data;
            //设置定时(秒)
            if (
              localStorage.getItem("examDuration" + this.examInfo.examId) ===
              "0"
            )
              localStorage.removeItem("examDuration" + this.examInfo.examId);
            this.duration =
              localStorage.getItem("examDuration" + this.examInfo.examId) ||
              resp.data.data.examDuration * 60;
            //考试剩余时间定时器
    ...
    async restoreLocalAnswers() {
      ...
      try {
        const restored = await restoreAnswers(Number(this.examInfo.examId));
        restored.forEach((row) => {
          const idx = this.questionInfo.findIndex(
            (q) => q.questionId === row.questionId
          );
          if (idx !== -1) {
            this.$set(this.userAnswer, idx, row.answer);
          }
        });
      } catch (e) {
        console.error("恢复离线答案失败", e);
      }
    },
    cacheExamAssets() {
      ...
      this.questionInfo.forEach((q) => {
        (q.images || []).forEach((u) => urls.push(u));
        (q.answer || []).forEach((a) =>
          (a.images || []).forEach((u) => urls.push(u))
        );
      });
    ...
    persistAnswer(index) {
      ...
      if (!question || !this.examInfo.examId) return;
      recordAnswer({
        examId: Number(this.examInfo.examId),
        questionId: question.questionId,
        answer:
          this.userAnswer[index] === undefined ? "" : this.userAnswer[index],
      }).catch((err) => console.error("保存离线答案失败", err));
    },
    onTextAnswerChange(index, value) {
      this.$set(this.userAnswer, index, value);
      this.persistAnswer(index);
    },
    handleSubmitResponse(resp, successMsg) {
      if (resp && (resp.status === 202 || (resp.data && resp.data.queued))) {
        this.$notify({
          title: "提示",
          message: "离线提交已排队，网络恢复后自动重放",
          type: "warning",
          duration: 3000,
        });
        return false;
      ...
      if (resp && resp.data && resp.data.code === 200) {
        ...
        clearExamState(Number(this.examInfo.examId || 0)).catch((e) =>
          console.error("清理离线缓存失败", e)
        );
        return true;
      }
      return false;
    ...
    async getQuestionInfo(ids) {
      const promises = ids.map((item, index) => {
        return this.$http
          .get(this.API.getQuestionById + "/" + item)
          .then((resp) => {
            if (resp.data.code === 200) {
      ...
      try {
        ...
        if (this.questionInfo.length < ids.length) {
          this.$message.warning(
            `部分题目加载失败，当前可用题目: ${this.questionInfo.length}/${ids.length}`
          );
        }
      } catch (error) {
      ...
      await this.restoreLocalAnswers();
      this.cacheExamAssets();
      this.loading.close();
      this.show = true;
    },
    //点击展示高清大图
    ...
    selectedMultipleAnswer(index) {
      if (this.userAnswer[this.curIndex] === undefined) {
        ...
        if (newArr.length === 0) {
          ...
          //答案格式化顺序DBAC -> ABCD
          this.userAnswer[this.curIndex] = String(
            this.userAnswer[this.curIndex]
          )
            .split(",")
            .sort(function (a, b) {
              return a - b;
    ...
    async verifyFaceDuringExam() {
      ...
      try {
        ...
        const res = await this.$http.post(
          `${this.API.faceVerifyExam}?userId=${
            this.examInfo.userId || localStorage.getItem("userId")
          }&examId=${this.examInfo.examId}`,
          { image: imageBase64 }
        );
        ...
        if (res.data.code === 200 && res.data.data.matched) {
          // 验证通过
          console.log(
            `第${this.faceVerifyCount}次人脸验证通过，相似度：${res.data.data.similarity}`
          );
          ...
          this.$message.error(
            `人脸验证失败（${this.faceVerifyFailCount}/${this.maxFaceVerifyFail}），请确保本人在考试`
          );
    ...
    async forceSubmitExam(reason) {
      ...
      this.$http.post(this.API.addExamRecord, data).then((resp) => {
        const ok = this.handleSubmitResponse(resp, `考试已结束：${reason}`);
        if (ok && resp.data && resp.data.data) {
    ...
    async uploadExamToAdmin() {
      ...
      this.userAnswer.forEach((item, index) => {
        if (this.questionInfo[index].questionType === 4) {
          //简答题答案处理
      ...
      if (!flag) {
        ...
        this.$confirm("当前试题暂未做完, 是否继续提交o(╥﹏╥)o ?", "提示", {
          ...
          .then(() => {
            ...
            data.questionIds = data.questionIds.join(",");
            this.$http.post(this.API.addExamRecord, data).then((resp) => {
              const ok = this.handleSubmitResponse(resp, "考试结束 *^▽^*");
              if (ok && resp.data && resp.data.data) {
                if (this.cameraOn) {
                  this.closeCamera();
                }
  ...
  watch: {
    ...
    duration(newVal) {
      ...
      if (newVal < 1) {
        ...
        data.questionIds = data.questionIds.join(",");
        this.$http.post(this.API.addExamRecord, data).then((resp) => {
          const ok = this.handleSubmitResponse(
            resp,
            "考试时间结束,已为您自动提交*^▽^*"
          );
          if (ok && resp.data && resp.data.data) {
