...
/**
 * ============================================================
 * 技术亮点5: JWT + SHA-256 认证工具类
 * ============================================================
 * 功能说明：
 * 1. 使用JWT（JSON Web Token）实现无状态认证
 * 2. 采用HMAC-SHA256算法对Token进行签名，防止篡改
 * 3. Token中包含用户信息（角色、用户名、头像等），无需查询数据库
 * 4. 设置过期时间（默认5小时），自动失效保证安全性
 * 
 * JWT优势：
 * - 无状态：服务端不需要存储Session，易于水平扩展
 * - 跨域友好：适合前后端分离架构
 * - 性能高：验证只需计算签名，无需查询数据库
 * - 自包含：Token中包含用户信息，减少数据库查询
 * 
 * 安全机制：
 * - HMAC-SHA256签名：防止Token被篡改
 * - 过期时间：限制Token有效期，降低泄露风险
 * - 密钥保护：使用私钥签名，只有服务端能生成有效Token
 * 
 * 业务场景：
 * - 用户登录成功后，生成Token返回给前端
 * - 前端每次请求携带Token（放在Authorization头）
 * - 后端拦截器验证Token，获取用户信息
 * 
