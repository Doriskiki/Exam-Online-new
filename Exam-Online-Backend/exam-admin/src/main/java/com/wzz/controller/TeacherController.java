...
//                    }
    ...
    public CommonResult<List<Exam>> getExamInfo(@RequestBody ExamQueryVo examQueryVo) {
        ...
        Integer pageNo = examQueryVo.getPageNo() != null ? examQueryVo.getPageNo() : 1;
        Integer pageSize = examQueryVo.getPageSize() != null ? examQueryVo.getPageSize() : 10;
        ...
        if (examQueryVo.getExamName() != null && !examQueryVo.getExamName().isEmpty()) {
            wrapper.like("exam_name", examQueryVo.getExamName());
        }
        // Add null-safety checks for startTime
        if (examQueryVo.getStartTime() != null && !examQueryVo.getStartTime().isEmpty()) {
            String startTime = examQueryVo.getStartTime();
            if (startTime.contains("T")) {
                wrapper.gt("start_time", startTime.substring(0, startTime.indexOf("T")));
            } else {
                wrapper.gt("start_time", startTime);
            }
        }
        // Add null-safety checks for endTime
        if (examQueryVo.getEndTime() != null && !examQueryVo.getEndTime().isEmpty()) {
            String endTime = examQueryVo.getEndTime();
            if (endTime.contains("T")) {
                wrapper.lt("end_time", endTime.substring(0, endTime.indexOf("T")));
            } else {
    ...
    public CommonResult<Object> getExamInfoById(@RequestParam Integer examId) {
        ...
        // Validate examId parameter
        if (examId == null) {
            return new CommonResult<>(400, "考试ID不能为空");
        }
        ...
        if (redisUtil.get("examInfo:" + examId) != null) {
            ...
            if (exam == null) {
                return new CommonResult<>(404, "考试不存在");
            }
            ...
            // Check if exam has questions - exam must have questions to start
            if (examQuestion == null) {
                log.warn("考试ID: {} 没有关联的题目", examId);
                return new CommonResult<>(400, "该考试还未添加题目，无法开始考试");
            }
